...
async function startServer() {
  ...
  app.get('/auth/google/url', (req, res) => {
    // Construct the redirect URI using the application's current origin
  ...
  app.post('/api/upload-avatar', async (req, res) => {
    try {
      ...
      // Get or create Avatar folder inside GOOGLE_DRIVE_FOLDER_ID (falls back to root)
      ...
      // Make the file public (Anyone with link can read)
      await drive.permissions.create({
        fileId: file.data.id!,
        requestBody: {
          role: 'reader',
      ...
      // Direct url to render in img tags via our backend proxy
  ...
  app.put('/api/users/:id', async (req, res) => {
    ...
    try {
      ...
      // 1. Fetch current users to find row index
      ...
      // 2. Prepare the updated row
      const currentRow = rows[rowIndex];
      // Pad to length 10 if necessary
  ...
  app.put('/api/users/:id/password', async (req, res) => {
    ...
    try {
      ...
      const response = await sheets.spreadsheets.values.get({
        spreadsheetId: SPREADSHEET_ID,
        range: 'users!A:C', // Columns A: userID, B: userName, C: password
      });
      ...
      // Write the new password directly to column C (row index rowIndex + 1)
      await sheets.spreadsheets.values.update({
        spreadsheetId: SPREADSHEET_ID,
  ...
  app.post('/api/login', async (req, res) => {
    ...
    try {
      ...
      // Headers: userID(0), userName(1), password(2), firstName(3), lastName(4), role(5), team(6), status(7), avatarUrl(8), email(9)
  ...
  app.get('/api/data', async (req, res) => {
    try {
      ...
      // Map Users (Sanitize: skip password at index 2)
      ...
      res.json({
        ...
        deliverables: [] // Currently empty in spreadsheet fetch logic
      });
    } catch (error: any) {
  ...
  app.post('/api/transmittals', async (req, res) => {
    try {
      ...
      // Get current IDs to find the next sequence for THIS month/year
      const response = await sheets.spreadsheets.values.get({
        spreadsheetId: SPREADSHEET_ID,
      ...
      // Filter IDs that match the current month-year prefix
      const monthIds = rows
      ...
      if (monthIds.length > 0) {
        // Extract the numbers from the end of matching IDs
  ...
  app.post('/api/clients', async (req, res) => {
    ...
    try {
      ...
      if (rows.length > 0) {
        // Filter out headers or empty rows and find max numeric ID
  ...
  app.put('/api/clients/:id', async (req, res) => {
    ...
    try {
      ...
      // 1. Find the row index for this client
      const response = await sheets.spreadsheets.values.get({
        spreadsheetId: SPREADSHEET_ID,
        range: 'clients!A:A',
      });
      ...
      // 2. Map frontend names to sheet columns
      // A:id, B:name, C:tin, D:entityType, E:email, F:contactPerson, G:status, H:fiscalYearEnd
      const row = [
        id.padStart(4, '0'),
        updatedClient.name || '',
        updatedClient.tin || '',
        updatedClient.entityType || updatedClient.entity_type || '', // Handle both camel and snake case just in case
        updatedClient.email || '',
  ...
  app.put('/api/retainers/:id', async (req, res) => {
    ...
    try {
      ...
      // 1. Update retainerEngagements Row
      ...
      // Update row: retainerID, clientID, serviceID, startDate, status, assignedStaffID
      ...
      // 2. Clear existing deadlines for this retainer
  ...
  app.delete('/api/retainers/:id', async (req, res) => {
    ...
    try {
      ...
      // 1. Remove from retainerEngagements
      const rResponse = await sheets.spreadsheets.values.get({
        spreadsheetId: SPREADSHEET_ID,
        range: 'retainerEngagements!A:F',
      });
  ...
  app.post('/api/retainers', async (req, res) => {
    ...
    try {
      ...
      for (const task of assignments) {
        ...
        // 2. Add Retainer Row
        // Header: retainerID, clientID, serviceID, startDate, status, assignedStaffID
        await sheets.spreadsheets.values.append({
          spreadsheetId: SPREADSHEET_ID,
        ...
        if (serviceId === '0001' && selectedTaxes && selectedTaxes.length > 0) {
          // Multiple Tax Compliances
          deadlineRows = selectedTaxes.map((tax: any, idx: number) => [
            (nextDlIdNum + idx).toString().padStart(4, '0'),
            retainerId,
            serviceId,
            tax.taxID,
            tax.dueDateCode // e.g. "M+15"
          ]);
        } else if (dueDateCode) {
          // Single service deadline
          deadlineRows = [[
            nextDlIdNum.toString().padStart(4, '0'),
            retainerId,
            serviceId,
  ...
  app.post('/api/specials', async (req, res) => {
    ...
    try {
      ...
      for (const task of assignments) {
        ...
        // Row: specialID clientID assignedStaffID serviceID projectTitle startDate endDate status description
        const row = [
          specialId,
  ...
  app.put('/api/specials/:id', async (req, res) => {
    ...
    try {
      ...
      // Preserve existing values for columns not provided in data
      const currentRow = rows[rowIndex];
      const updatedRow = [...currentRow];
      
      // Map: 0:specialID, 1:clientID, 2:assignedStaffID, 3:serviceID, 4:projectTitle, 5:startDate, 6:endDate, 7:status, 8:description
      ...
      // Ensure the ID itself is padded if we're writing it back
      updatedRow[0] = String(updatedRow[0] || '').padStart(4, '0');
      // Ensure clientID is padded
      updatedRow[1] = String(updatedRow[1] || '').padStart(4, '0');

      // Ensure row has at least 9 columns for the description
  ...
  app.put('/api/retainer-logs', async (req, res) => {
    ...
    try {
      ...
      // 1. Find the row index for this deadline and period
      const response = await sheets.spreadsheets.values.get({
        spreadsheetId: SPREADSHEET_ID,
  ...
  app.put('/api/tasks/:id', async (req, res) => {
    ...
    try {
      ...
      await sheets.spreadsheets.values.update({
        spreadsheetId: SPREADSHEET_ID,
        range: `taskLog!A${rowIndex + 2}:D${rowIndex + 2}`, // +2 because header is row 1 and index is 0-based
        valueInputOption: 'USER_ENTERED',
        requestBody: { values: [updatedRow] }
      });
  ...
  app.delete('/api/tasks/:id', async (req, res) => {
    ...
    try {
      ...
      // 2. Cascading Delete from activityLog (all rows where taskID matches)
      const activityResponse = await sheets.spreadsheets.values.get({
        spreadsheetId: SPREADSHEET_ID,
        range: 'activityLog!A2:B', // Column B is taskID
      });
      ...
      activityRows.forEach((row, index) => {
        if (row[1] === id) {
          indicesToDelete.push(index + 1); // +1 because range starts at A2
        }
      });
  ...
  app.delete('/api/activities/:id', async (req, res) => {
    ...
    try {
      ...
      // 2. Get the sheetId for 'activityLog'
          ...
          requests: [
            {
              deleteDimension: {
                range: {
                  sheetId: sheetId,
                  dimension: 'ROWS',
                  startIndex: rowIndex + 1, // +1 because we started range at A2
                  endIndex: rowIndex + 2
                }
              }
            }
